...
This is **RC12**. Both directions are implemented, but physical GPU/audio/device and router compatibility must still be measured on your own hardware. CI validates the encrypted media protocols, H.264 encode/decode plumbing, native desktop capture/input, Android build/lint and lifecycle behavior that can run without bypassing Android's mandatory projection/accessibility consent. Android control first uses semantic Accessibility click/scroll actions and falls back to physical gestures, with queue recovery and control-result diagnostics. Direct Internet mode currently works where the computer's router provides UPnP and a genuine public IPv4 address. P2PShare's STUN/UDP punch has been verified as a useful route-discovery design, but it cannot carry HandOff's ordered TLS byte stream until the runtime moves that stream onto a UDP transport such as QUIC; RC12 does not falsely label UPnP as hole punching. CGNAT, symmetric NAT, disabled UPnP and restrictive firewalls may still require a relay or VPN. Per-app desktop audio isolation, tightly synchronized A/V, file drag-and-drop and native Wayland remain outside this build. Linux remote text entry is not advertised because portable Unicode injection is not reliable across X11 keyboard maps.
